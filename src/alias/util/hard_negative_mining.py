...
def mine_hard_negatives(
    ...
    use_gpu: bool = False, # only for usage on cuda
    ...
    Args:
        ...
        output_format (Literal["triplet", "n-tuple", "labeled-pair", "labeled-list"]): Output format for the `datasets.Dataset`. Options are:
            ...
            - "paired-triplet": (anchor, positive, negative) triplets, i.e. 3 columns, but assignes one negative per achor-positve pair to avoid triplet explosion. Usage same as "triplet".
    ...
    if label_column_name is not None:
        labels = list(dataset[label_column_name])
    ...
    if use_faiss:
        ...
        # Compute the similarity scores between the queries and the corpus (using GPU) - only works on cuda
        query_embeddings = torch.from_numpy(query_embeddings).to(device).half()
        corpus_embeddings = torch.from_numpy(corpus_embeddings).to(device).half()
        
        # Normalize embeddings for cosine similarity
        query_embeddings = F.normalize(query_embeddings, p=2, dim=1)
        corpus_embeddings = F.normalize(corpus_embeddings, p=2, dim=1)
        
        # Compute the similarity (cosine similarity) between query embeddings and corpus embeddings
        scores = torch.matmul(query_embeddings, corpus_embeddings.T)  # Dot product (cosine similarity)

        # Keep only the range_max + max_positives highest scores
        actual_k = min(range_max + 1, scores.size(1))  # +1 to include the positive pair
        scores, indices = torch.topk(scores, k=actual_k, dim=1)
        ...
        actual_k = min(range_max, scores.size(1))
        scores, indices = torch.topk(scores, k=actual_k, dim=1)
    ...
    if label_column_name is not None:
        label_indices = [[] for _ in range(n_queries)]

        # Step 1: Build query → label mapping (handle duplicates safely)
        query_labels = defaultdict(set)
        for query, label in zip(all_queries, labels):
            query_labels[query].add(label)

        # Flatten: warn if any query has multiple labels
        for query, label_set in query_labels.items():
            if len(label_set) > 1:
                logger.warning(f"Query '{query}' has multiple labels: {label_set}. Using the first one.")
            query_labels[query] = next(iter(label_set))

        # Step 2: Build corpus → label mapping 
        corpus_labels = {}
        for doc, label in zip(all_queries, labels):
            corpus_labels[doc] = label
        for doc, label in zip(positives, labels):
            corpus_labels[doc] = label

        # Step 3: Build label → corpus indices mapping 
        label_to_corpus_indices = defaultdict(list)
        for doc in corpus:
            label = corpus_labels.get(doc, None)
            if label is None:
                logger.warning(f"No label found for corpus doc '{doc}'. Skipping.")
                continue
            idx = corpus_idx.get(doc, None)
            if idx is None:
                logger.warning(f"Doc '{doc}' not found in corpus_idx. Skipping.")
                continue
            label_to_corpus_indices[label].append(idx)

        # Step 4: Group query indices by label
        label_to_query_indices = defaultdict(list)
        for query in queries:
            label = query_labels.get(query, None)
            if label is None:
                logger.warning(f"No label found for query '{query}'. Skipping.")
                continue
            q_idx = queries_idx[query]
            label_to_query_indices[label].append(q_idx)

        # Step 5: Assign label-based corpus indices to all queries in the same group
        for label, query_indices in tqdm(label_to_query_indices.items(), desc="Assigning label_indices"):
            same_label_docs = label_to_corpus_indices.get(label, [])
            for q_idx in query_indices:
                label_indices[q_idx] = same_label_docs

        # Step 6: Convert to tensors on device
        label_indices = [
            torch.tensor(p if p else [-1], device=device)  # use -1 as placeholder for empty
    ...
    if not include_positives and label_column_name is None:
    ...
    if label_column_name is not None:
        ...
        label_mask = torch.stack(
            [torch.isin(indices[q_idx], label_indices[q_idx]) for q_idx in range(n_queries)]
        )

        # Exclude negatives with same label as query
        scores[label_mask] = -float("inf")
    ...
    actual_k = min(range_max, scores.size(1))
    negative_scores, local_indices = torch.topk(scores, k=actual_k, dim=1)
    ...
    if output_format != 'paired-triplet':
    ...
    if output_format == "triplet":
        ...
        difference_scores = positive_scores.repeat(num_negatives, 1).T[indices_to_keep] - negative_scores
    ...
    elif output_format == "paired-triplet":
            dataset_data = {
                anchor_column_name: [],
                positive_column_name: [],
                "negative": [],
            ...
            split_sizes = n_positives  # list of number of positives per query
            positive_scores_per_query = torch.split(positive_scores, split_sizes)

            for q_idx in range(n_queries):
                p_indices = positive_indices[q_idx]  # tensor of positive idxs
                p_scores = positive_scores_per_query[q_idx]
                n_scores = negative_scores[q_idx]
                n_indices = indices[q_idx]  # candidate negatives for this query

                valid_mask = n_scores != -float("inf")
                n_scores = n_scores[valid_mask]
                n_indices = n_indices[valid_mask]
                
                if len(n_indices) == 0:
                    continue 

                num_pos = min(len(p_indices), 5)  # Cap at 5 positives if needed

                if len(n_indices) < num_pos:
                    sampled_neg_indices = n_indices[torch.randint(0, len(n_indices), (num_pos,))]
                    sampled_neg_scores = n_scores[torch.randint(0, len(n_scores), (num_pos,))]
                else:
                    rand_perm = torch.randperm(len(n_indices))
                    sampled_neg_indices = n_indices[rand_perm[:num_pos]]
                    sampled_neg_scores = n_scores[rand_perm[:num_pos]]

                for i in range(num_pos):
                    pos_idx = p_indices[i]
                    neg_idx = sampled_neg_indices[i]

                    dataset_data[anchor_column_name].append(queries[q_idx])
                    dataset_data[positive_column_name].append(corpus[pos_idx])
                    dataset_data["negative"].append(corpus[neg_idx])

                    diff_score = p_scores[i] - sampled_neg_scores[i]
